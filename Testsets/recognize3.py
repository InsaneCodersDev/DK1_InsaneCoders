...
def test(thres=0.4, ambiguity=1, verbose=False):
    ...
    with open('files.csv', 'r') as f:
        ...
        for line in r:
            ...
            # print(line[0])
            sound = AudioSegment.from_file(line[0])
            sound = sound.set_frame_rate(8000)
            sound = sound.set_channels(1)
            sound = sound.set_sample_width(2)
            sound.export(line[0], format="wav")
            identity = recognize(line[0], thres, ambiguity)
            if identity == line[1]:
                if print_success:
                    if not verbose:
                        print(
                            f"\u001b[32m Success: {line[1]} was correctly recognized.")
                correct += 1
                ...
                if print_success:
                    if not verbose:
                        print(
                            f"\u001b[32m Success: Incorrect recording by {line[1]} was marked as not recognized.")
            elif identity == "Not identified" and line[2] == '1':
                false_negatives += 1
                if not verbose:
                    print(
                        f"\u001b[33m Partial Failure: {line[1]} was Marked as {identity}")
            else:
                if not verbose:
                    print(
                        f"\u001b[31m Failure: {line[1]} was incorrectly marked as {identity}")
            total += 1
